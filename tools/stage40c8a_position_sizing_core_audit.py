...
REPO = Path(".").resolve()
OUT = REPO / "artifacts/stage40c8a_position_sizing_core_audit"
...
EXPECTED_CORE = REPO / "src/signalforge/engines/portfolio_construction/position_sizing.py"
...
MARKERS = [
    ...
    "return bound",
    "return-bound",
    "portfolio_position_sizing",
    "position_sizing_replay",
    "capital_sufficiency",
    "quantity",
    "notional",
    "max_loss",
...
FOCUS_FUNCTIONS = {
    ...
    "build_portfolio_position_sizing",
    "build_from_paths",
    "position_sizing",
    "size_position",
...
def discover_files() -> list[Path]:
    ...
    for root in SEARCH_ROOTS:
        ...
        for path in root.rglob("*.py"):
            ...
            name_hit = any(h in path.name.lower() for h in TARGET_NAME_HINTS)

            # Keep this narrower than the expectancy/pruned audits.
            # We want actual sizing/capital/exposure files, not every file that mentions allocation.
            if marker_hits and name_hit:
                files.append(path)
...
def classify(path: Path) -> str:
    ...
    if "portfolio_position_sizing_replay.py" == name:
        return "24_position_sizing_core_candidate"

    if "position_sizing" in name:
        return "24_position_sizing_related"

    if "capital_sufficiency" in name:
        return "24_capital_sufficiency_related"

    if "rule_sensitivity" in name or "exposure_constraint" in name or "return_bound" in name:
        return "24_sensitivity_or_validation_related"

    return "position_sizing_related_unknown"
...
def main() -> int:
    ...
    for path in files:
        ...
        elif stage_guess == "24_position_sizing_core_candidate":
            verdict = "not_verified_position_sizing_logic_owned_by_builder"
        elif stage_guess.startswith("24_") and (focus_count > 0 or focus_hit_count > 60):
            verdict = "not_verified_position_sizing_or_validation_logic_owned_by_builder"
        else:
        ...
        reports.append({
            ...
            "verdict": verdict,
            "marker_hits": marker_hits_for(path),
    ...
    production_candidates = [
        r for r in reports
        if r["stage_guess"] == "24_position_sizing_core_candidate"
    ]

    summary = {
        "adapter_type": "stage40c8a_position_sizing_core_auditor",
        "artifact_type": "signalforge_stage40c8a_position_sizing_core_audit",
        "contract": "stage40c8a_position_sizing_core_audit",
        "is_ready": len(blocker_reports) == 0 and EXPECTED_CORE.exists() and total_core_import_hits > 0 and total_core_call_hits > 0,
        "stage": "24_position_sizing_return_bound_sensitivity",
        ...
        "paths": {
            "summary_path": "artifacts/stage40c8a_position_sizing_core_audit/signalforge_stage40c8a_position_sizing_core_audit_summary.json",
            "detail_path": "artifacts/stage40c8a_position_sizing_core_audit/signalforge_stage40c8a_position_sizing_core_audit_detail.json",
        },
    }
    ...
    (OUT / "signalforge_stage40c8a_position_sizing_core_audit_summary.json").write_text(
        json.dumps(summary, indent=2, sort_keys=True),
        encoding="utf-8",
    )

    (OUT / "signalforge_stage40c8a_position_sizing_core_audit_detail.json").write_text(
