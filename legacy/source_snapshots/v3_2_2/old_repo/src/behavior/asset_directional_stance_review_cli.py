...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Build SignalForge asset directional stance review/export artifact."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=25,
        help="Number of items to include in each review list.",
    )
    ...
    result = build_signalforge_asset_directional_stance_review(
        source,
        top_n=args.top_n,
    )

    summary = write_asset_directional_stance_review_result(
        result=result,
        output_dir=args.output_dir,
    )

    summary_path = Path(summary["files"]["summary"])
    result_path = Path(summary["files"]["asset_directional_stance_review_result"])

    summary["file_summary"]["file_sizes"]["summary"] = (
        summary_path.stat().st_size if summary_path.exists() else 0
    )
    summary["file_summary"]["file_sizes"]["asset_directional_stance_review_result"] = (
