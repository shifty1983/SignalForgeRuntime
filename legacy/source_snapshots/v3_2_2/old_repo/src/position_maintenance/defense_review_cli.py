...
from src.position_maintenance.defense_review_file_writer import (
    write_options_strategy_defense_review_operation_files,
)


CLI_SUMMARY_SCHEMA_VERSION = "options_strategy_defense_review_cli_summary.v1"
DEFAULT_CLI_SUMMARY_FILENAME = "options_strategy_defense_review_cli_summary.json"
...
def main(
    ...
    *,
    writer: WriterFunction = write_options_strategy_defense_review_operation_files,
) -> int:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Build local options strategy defense review operation artifacts "
            "from existing defined-risk options positions. This command writes "
            "local files only and does not call brokers, route orders, submit "
            "orders, model fills, perform live execution, model slippage, or "
            "create automatic close/roll/defense orders."
        )
    )
    parser.add_argument(
        "--source",
        required=True,
        help="Path to options strategy defense review source JSON.",
    )
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Directory where defense review artifacts should be written.",
    )
    parser.add_argument(
        "--plan-date",
        default=None,
    ...
    parser.add_argument(
        "--market-regime",
        default=None,
        help="Optional market regime override. If omitted, source.market_regime is used.",
    )
    parser.add_argument(
        "--max-candidates-per-position",
        type=int,
        default=None,
        help="Optional cap for defense candidates retained per position.",
    )
    return parser
...
def _build_cli_summary(
    *,
    ...
    return {
        ...
        "operation_type": "options_strategy_defense_review_cli",
