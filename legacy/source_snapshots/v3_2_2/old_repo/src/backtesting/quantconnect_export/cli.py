...
from src.backtesting.quantconnect_export.file_writer import (
    write_quantconnect_export_files,
)


CLI_SUMMARY_SCHEMA_VERSION = "quantconnect_export_cli_summary.v1"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = write_quantconnect_export_files(
        source,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Write QuantConnect/LEAN export artifacts from a SignalForge JSON source. "
            "This command writes local files only and does not call QuantConnect, "
            "brokers, market-data APIs, order-routing systems, live execution systems, "
            "fill engines, or slippage engines."
        )
    )
    ...
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Directory where QuantConnect export artifacts should be written.",
    )
