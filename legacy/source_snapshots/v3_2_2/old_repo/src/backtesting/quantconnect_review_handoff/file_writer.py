...
FILE_WRITER_SCHEMA_VERSION = "quantconnect_review_handoff_files.v1"
OPERATION_TYPE = "quantconnect_review_handoff_file_writer"

DEFAULT_FILENAMES = {
    "handoff_bundle": "quantconnect_review_handoff_bundle.json",
    "operation_result": "quantconnect_review_handoff_operation.json",
    "audit_report": "quantconnect_review_handoff_audit.json",
    "health_report": "quantconnect_review_handoff_health.json",
    "ready_payloads": "quantconnect_review_handoff_ready_payloads.json",
    "needs_review_payloads": "quantconnect_review_handoff_needs_review_payloads.json",
    "blocked_payloads": "quantconnect_review_handoff_blocked_payloads.json",
    "event_log": "quantconnect_review_handoff_operation.jsonl",
}


def write_quantconnect_review_handoff_files(
    review_summary_operation_result: Any,
    *,
    output_dir: str | PathLike[str],
) -> dict[str, Any]:
    """Write QuantConnect review handoff artifacts to local JSON/JSONL files.
    ...
    or external data warehouses. It only writes deterministic local handoff files
    from an existing review summary operation artifact.
    """
    ...
    operation_result = run_quantconnect_review_handoff_operation(
        review_summary_operation_result,
        event_log_path=event_log_path,
    )

    handoff_bundle = _as_mapping(operation_result.get("handoff_bundle"))
    audit_report = _as_mapping(operation_result.get("audit_report"))
    health_report = _as_mapping(operation_result.get("health_report"))

    files = {
        "handoff_bundle": output_path / DEFAULT_FILENAMES["handoff_bundle"],
    ...
    _write_json(files["handoff_bundle"], handoff_bundle)
