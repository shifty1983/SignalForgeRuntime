...
CLI_SUMMARY_SCHEMA_VERSION = "quantconnect_result_import_cli_summary.v1"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = write_quantconnect_result_import_files(
        source,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Import manually supplied QuantConnect backtest results/logs into "
            "SignalForge result artifacts. This command writes local files only "
    ...
    parser.add_argument(
        "--source",
        required=True,
        help="Path to the manually saved QuantConnect result JSON.",
    )
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Directory where QuantConnect result import artifacts should be written.",
    )
