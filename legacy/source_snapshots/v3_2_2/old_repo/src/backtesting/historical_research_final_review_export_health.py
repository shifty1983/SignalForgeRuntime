# src/backtesting/historical_research_final_review_export_health.py
...
def evaluate_historical_research_final_review_export_health(
    ...
    *,
    ...
    if export_status == "ready":
        if ready_count <= 0:
            blocked_reasons.append("ready final review export has no ready payloads")

        if needs_review_count > 0:
            warnings.append("ready final review export has needs_review payloads")

        if blocked_count > 0:
            warnings.append("ready final review export has blocked payloads")

    if total_count == 0 and export_status != "blocked":
        warnings.append("final review export has no payloads")
