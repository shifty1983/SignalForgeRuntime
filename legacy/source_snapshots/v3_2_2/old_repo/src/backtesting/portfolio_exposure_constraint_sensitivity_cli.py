"""Portfolio exposure constraint sensitivity replay.

This CLI replays the enriched SignalForge portfolio ledger while applying portfolio
exposure constraints on top of the already-validated live-realism profile. It is
intended for Phase 8 rule refinement and deliberately does not rebuild expectancy,
reselect strategies, or change entry/exit logic.

Tested constraint families:
- max trades per realization/decision date
- max trades per symbol per month
- max trades per strategy per month

The objective is not to maximize historical return. The objective is to test whether
simple concentration controls reduce portfolio dependency without destroying the
validated edge.
"""
...
ADAPTER_TYPE = "portfolio_exposure_constraint_sensitivity_builder"
ARTIFACT_TYPE = "signalforge_portfolio_exposure_constraint_sensitivity"
CONTRACT = "portfolio_exposure_constraint_sensitivity"
...
@dataclass(frozen=True)
class ExposureRule:
    max_trades_per_date: int | None
    max_symbol_trades_per_month: int | None
    max_strategy_trades_per_month: int | None

    @staticmethod
    def _label(value: int | None) -> str:
        return "unlimited" if value is None else str(value)

    @property
    def name(self) -> str:
        return (
            f"date_{self._label(self.max_trades_per_date)}"
            f"_symbol_month_{self._label(self.max_symbol_trades_per_month)}"
            f"_strategy_month_{self._label(self.max_strategy_trades_per_month)}"
        )
...
def _parse_optional_int_list(value: str) -> list[int | None]:
    ...
    for part in value.split(","):
        ...
        if not text:
            continue
        if text in {"0", "none", "null", "unlimited", "inf", "infinite"}:
            out.append(None)
        else:
            number = int(text)
            if number <= 0:
                ...
                out.append(number)
    if not out:
        raise argparse.ArgumentTypeError("expected at least one integer or unlimited value")
    return out
...
def _calculate_score(row: dict[str, Any]) -> float:
    if not row.get("passes_exposure_constraint_gate"):
        return -1.0
    ...
    exposure_skip = float(row.get("exposure_constraint_skip_rate") or 0.0)
...
def _replay_scenario(
    *,
    ...
    skipped_exposure_date_count = 0
    skipped_exposure_symbol_month_count = 0
    skipped_exposure_strategy_month_count = 0
    ...
    trades_by_date: Counter[str] = Counter()
    trades_by_symbol_month: Counter[tuple[str, str]] = Counter()
    trades_by_strategy_month: Counter[tuple[str, str]] = Counter()
    ...
    for trade in trades:
        ...
        if exposure_rule.max_trades_per_date is not None and trades_by_date[date] >= exposure_rule.max_trades_per_date:
            skipped_exposure_date_count += 1
            continue
        if (
            exposure_rule.max_symbol_trades_per_month is not None
            and trades_by_symbol_month[(symbol, month)] >= exposure_rule.max_symbol_trades_per_month
        ):
            skipped_exposure_symbol_month_count += 1
            continue
        if (
            exposure_rule.max_strategy_trades_per_month is not None
            and trades_by_strategy_month[(strategy, month)] >= exposure_rule.max_strategy_trades_per_month
        ):
            skipped_exposure_strategy_month_count += 1
            continue

        budget = min(equity * policy.risk_per_trade_pct, policy.max_trade_risk_dollars)
        ...
        trades_by_symbol_month[(symbol, month)] += 1
        trades_by_strategy_month[(strategy, month)] += 1

        by_symbol[symbol] += net_pnl
        by_strategy[strategy] += net_pnl
        ...
        equity_rows.append(
            {
                ...
                "max_trades_per_date": exposure_rule.max_trades_per_date,
                "max_symbol_trades_per_month": exposure_rule.max_symbol_trades_per_month,
                "max_strategy_trades_per_month": exposure_rule.max_strategy_trades_per_month,
                "date": date,
                "symbol": symbol,
                "strategy": strategy,
    ...
    exposure_skipped_count = skipped_exposure_date_count + skipped_exposure_symbol_month_count + skipped_exposure_strategy_month_count
    exposure_constraint_skip_rate = exposure_skipped_count / attempted_trade_count if attempted_trade_count else 0.0
    ...
    row: dict[str, Any] = {
        "scenario_name": f"capital_{starting_capital:g}_{exposure_rule.name}",
        "capital_scenario": starting_capital,
        "starting_capital": starting_capital,
        "risk_per_trade_pct": policy.risk_per_trade_pct,
        "max_trade_risk_dollars": policy.max_trade_risk_dollars,
        "max_trades_per_date": exposure_rule.max_trades_per_date,
        "max_symbol_trades_per_month": exposure_rule.max_symbol_trades_per_month,
        "max_strategy_trades_per_month": exposure_rule.max_strategy_trades_per_month,
        "exposure_rule_name": exposure_rule.name,
        ...
        "skipped_exposure_date_count": skipped_exposure_date_count,
        "skipped_exposure_symbol_month_count": skipped_exposure_symbol_month_count,
        "skipped_exposure_strategy_month_count": skipped_exposure_strategy_month_count,
        "exposure_skipped_count": exposure_skipped_count,
        "exposure_constraint_skip_rate": exposure_constraint_skip_rate,
        ...
        "passes_exposure_constraint_gate": passes,
        "exposure_constraint_gate_status": "pass" if passes else "fail",
...
def _best_by_capital(scenarios: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for capital, rows in grouped.items():
        passing = [row for row in rows if row.get("passes_exposure_constraint_gate")]
        ...
        output[capital] = {
            ...
            "max_trades_per_date": best["max_trades_per_date"],
            "max_symbol_trades_per_month": best["max_symbol_trades_per_month"],
            "max_strategy_trades_per_month": best["max_strategy_trades_per_month"],
            "passes_exposure_constraint_gate": best["passes_exposure_constraint_gate"],
            "robustness_score": best["robustness_score"],
            "total_return": best["total_return"],
            "max_drawdown_pct": best["max_drawdown_pct"],
            "profit_factor": best["profit_factor"],
            "trade_retention_rate": best["trade_retention_rate"],
            "exposure_constraint_skip_rate": best["exposure_constraint_skip_rate"],
            "top_symbol_positive_contribution_pct": best["top_symbol_positive_contribution_pct"],
            "top_strategy_positive_contribution_pct": best["top_strategy_positive_contribution_pct"],
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    exposure_rules = [
        ExposureRule(date_limit, symbol_month_limit, strategy_month_limit)
        for date_limit in args.max_trades_per_date_values
        for symbol_month_limit in args.max_symbol_trades_per_month_values
        for strategy_month_limit in args.max_strategy_trades_per_month_values
    ]

    scenarios: list[dict[str, Any]] = []
    equity_curve_rows: list[dict[str, Any]] = []
    for starting_capital in args.starting_capitals:
        for exposure_rule in exposure_rules:
    ...
    passing = [row for row in scenarios if row.get("passes_exposure_constraint_gate")]
    ...
    if not passing:
        warnings.append("no_exposure_constraint_scenarios_passed_quality_gate")

    scenarios_path = output_dir / "signalforge_portfolio_exposure_constraint_sensitivity_scenarios.jsonl"
    equity_curves_path = output_dir / "signalforge_portfolio_exposure_constraint_sensitivity_equity_curves.jsonl"
    summary_path = output_dir / "signalforge_portfolio_exposure_constraint_sensitivity_summary.json"
    thresholds_path = output_dir / "signalforge_portfolio_exposure_constraint_sensitivity_thresholds.json"
    ...
    summary: dict[str, Any] = {
        ...
        "risk_per_trade_pct": args.risk_per_trade_pct,
        "max_trade_risk_dollars": args.max_trade_risk_dollars,
        "max_trades_per_date_values": args.max_trades_per_date_values,
        "max_symbol_trades_per_month_values": args.max_symbol_trades_per_month_values,
        "max_strategy_trades_per_month_values": args.max_strategy_trades_per_month_values,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Replay portfolio exposure constraint sensitivity variants.")
    ...
    parser.add_argument("--max-trades-per-date-values", type=_parse_optional_int_list, default=[5, 10, 15, None])
    parser.add_argument("--max-symbol-trades-per-month-values", type=_parse_optional_int_list, default=[1, 2, 3, None])
    parser.add_argument("--max-strategy-trades-per-month-values", type=_parse_optional_int_list, default=[10, 20, 40, None])
