# src/backtesting/historical_research_plan_export_audit.py
...
def audit_historical_research_plan_export_record(
    ...
    if operation_type != "historical_research_plan_export":
        audit_errors.append(
    ...
    ready_research = operation_record.get("ready_research", [])
    ...
    expected_counts = {
        "ready_count": len(ready_research) if isinstance(ready_research, list) else 0,
        "needs_review_count": (
            len(needs_review_research)
            if isinstance(needs_review_research, list)
            else 0
        ),
        "blocked_count": (
            len(blocked_research) if isinstance(blocked_research, list) else 0
        ),
    }
    ...
    for key, expected_value in expected_counts.items():
        if export_summary.get(key) != expected_value:
            audit_errors.append(
                f"operation_record export_summary {key} does not match export payloads"
            )

    if operation_status == "completed" and expected_counts["total_count"] == 0:
        audit_warnings.append("operation_record completed with no export payloads")
