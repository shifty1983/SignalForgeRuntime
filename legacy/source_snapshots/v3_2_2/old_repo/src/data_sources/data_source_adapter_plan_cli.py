...
from src.signalforge.data_sources.data_source_adapter_plan_file_writer import (
    write_signalforge_data_source_adapter_plan_files,
)


CLI_SUMMARY_SCHEMA_VERSION = "signalforge_data_source_adapter_plan_cli_summary.v1"
DEFAULT_CLI_SUMMARY_FILENAME = "signalforge_data_source_adapter_plan_cli_summary.json"
...
def main(
    ...
    *,
    writer: WriterFunction = write_signalforge_data_source_adapter_plan_files,
) -> int:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Write the SignalForge data-source adapter plan. This command combines "
            "the inventory and contract registry into a prioritized adapter backlog. "
            "It does not call brokers, route orders, submit orders, model fills, "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional JSON source containing inventory_source and contracts_source. "
            "If omitted, the default unresolved adapter plan is written."
        ),
    )
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Directory where data-source adapter plan artifacts should be written.",
    )
    return parser
...
def _build_cli_summary(
    *,
    ...
    adapter_plan = writer_result.get("adapter_plan", {})

    return {
        "schema_version": CLI_SUMMARY_SCHEMA_VERSION,
        "operation_type": "signalforge_data_source_adapter_plan_cli",
        ...
        "inventory_status": summary.get("inventory_status"),
        "contracts_status": summary.get("contracts_status"),
        "adapter_plan_summary": summary.get("adapter_plan_summary", {}),
        "open_inventory_decision_count": summary.get("open_inventory_decision_count"),
        "open_adapter_source_count": summary.get("open_adapter_source_count"),
        "resolved_adapter_source_count": summary.get("resolved_adapter_source_count"),
        "recommended_next_adapter": summary.get("recommended_next_adapter", {}),
