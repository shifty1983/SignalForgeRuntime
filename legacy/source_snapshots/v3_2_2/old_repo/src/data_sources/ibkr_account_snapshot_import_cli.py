...
from src.signalforge.data_sources.ibkr_account_snapshot_import_file_writer import (
    write_signalforge_ibkr_account_snapshot_import_files,
)


CLI_SUMMARY_SCHEMA_VERSION = "signalforge_ibkr_account_snapshot_import_cli_summary.v1"
DEFAULT_CLI_SUMMARY_FILENAME = "signalforge_ibkr_account_snapshot_import_cli_summary.json"
...
def main(
    ...
    *,
    writer: WriterFunction = write_signalforge_ibkr_account_snapshot_import_files,
) -> int:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Normalize a local IBKR/manual account export into SignalForge "
            "account snapshot evidence. This command reads local JSON only. "
            "It does not call IBKR, call brokers, route orders, submit orders, "
            "model fills, perform live execution, model slippage, create "
    ...
    parser.add_argument(
        "--source",
        required=True,
        help="Path to local JSON source containing ibkr_statement, account_snapshot, payload, or flat fields.",
    )
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Directory where IBKR account snapshot import artifacts should be written.",
    )
    return parser
...
def _build_cli_summary(
    *,
    ...
    return {
        ...
        "operation_type": "signalforge_ibkr_account_snapshot_import_cli",
        ...
        "position_summary": summary.get("position_summary", {}),
