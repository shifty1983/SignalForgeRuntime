...
from src.options_portfolio.edge_validation_summary_file_writer import (
    write_options_edge_validation_summary_operation_files,
)


CLI_SUMMARY_SCHEMA_VERSION = "options_edge_validation_summary_cli_summary.v1"
DEFAULT_CLI_SUMMARY_FILENAME = "options_edge_validation_summary_cli_summary.json"
...
def main(
    ...
    *,
    writer: WriterFunction = write_options_edge_validation_summary_operation_files,
) -> int:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Build local options edge-validation summary operation artifacts from "
            "manual options outcome records. This command writes local files only "
            "and does not call brokers, route orders, submit orders, model fills, "
            "perform live execution, model slippage, or create automatic "
    ...
    parser.add_argument(
        "--source",
        required=True,
        help="Path to options edge-validation summary source JSON.",
    )
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Directory where options edge-validation summary artifacts should be written.",
    )
    parser.add_argument(
        "--summary-date",
        default=None,
        help=(
            "Optional summary date override. If omitted, source.summary_date is used."
        ),
    )
    return parser
...
def _build_cli_summary(
    *,
    ...
    return {
        ...
        "operation_type": "options_edge_validation_summary_cli",
