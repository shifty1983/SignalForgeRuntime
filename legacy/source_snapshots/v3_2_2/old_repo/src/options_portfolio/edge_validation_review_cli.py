...
from src.options_portfolio.edge_validation_review_file_writer import (
    write_options_edge_validation_review_operation_files,
)


CLI_SUMMARY_SCHEMA_VERSION = "options_edge_validation_review_cli_summary.v1"
DEFAULT_CLI_SUMMARY_FILENAME = "options_edge_validation_review_cli_summary.json"
...
def main(
    ...
    *,
    writer: WriterFunction = write_options_edge_validation_review_operation_files,
) -> int:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Build local options edge-validation review operation artifacts from "
            "options edge-validation summaries. This command writes local files only "
    ...
    parser.add_argument(
        "--source",
        required=True,
        help="Path to options edge-validation review source JSON.",
    )
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Directory where options edge-validation review artifacts should be written.",
    )
    parser.add_argument(
        "--review-date",
        default=None,
        help="Optional review date override. If omitted, source.review_date is used.",
    )
    return parser
...
def _build_cli_summary(
    *,
    ...
    return {
        ...
        "operation_type": "options_edge_validation_review_cli",
