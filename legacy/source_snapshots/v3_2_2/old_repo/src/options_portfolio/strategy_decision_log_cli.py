...
from src.options_portfolio.strategy_decision_log_file_writer import (
    write_options_strategy_decision_log_operation_files,
)


CLI_SUMMARY_SCHEMA_VERSION = "options_strategy_decision_log_cli_summary.v1"
DEFAULT_CLI_SUMMARY_FILENAME = "options_strategy_decision_log_cli_summary.json"
...
def main(
    ...
    *,
    writer: WriterFunction = write_options_strategy_decision_log_operation_files,
) -> int:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Build local options strategy decision log artifacts from options "
            "strategy improvement review outputs and human decisions. This command "
    ...
    parser.add_argument(
        "--source",
        required=True,
        help="Path to options strategy decision log source JSON.",
    )
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Directory where options strategy decision log artifacts should be written.",
    )
    parser.add_argument(
        "--log-date",
        default=None,
        help="Optional log date override. If omitted, source.log_date is used.",
    )
    return parser
...
def _build_cli_summary(
    *,
    ...
    return {
        ...
        "operation_type": "options_strategy_decision_log_cli",
