...
CLI_SUMMARY_SCHEMA_VERSION = "options_portfolio_control_report_cli_summary.v1"
DEFAULT_CLI_SUMMARY_FILENAME = "options_portfolio_control_report_cli_summary.json"
...
def main(
    ...
    *,
    writer: WriterFunction = write_options_portfolio_control_report_operation_files,
) -> int:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Build local top-level options portfolio control report artifacts from "
            "weekly plan, risk monitor, manual action, outcome, edge validation, "
            "strategy improvement, and strategy decision log artifacts. This command "
    ...
    parser.add_argument(
        "--source",
        required=True,
        help="Path to options portfolio control report source JSON.",
    )
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Directory where options portfolio control report artifacts should be written.",
    )
    parser.add_argument(
        "--report-date",
        default=None,
...
def _build_cli_summary(
    *,
    ...
    return {
        ...
        "operation_type": "options_portfolio_control_report_cli",
