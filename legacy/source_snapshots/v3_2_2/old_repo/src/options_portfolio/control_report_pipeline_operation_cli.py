...
CLI_SUMMARY_SCHEMA_VERSION = "options_portfolio_control_report_pipeline_operation_cli_summary.v1"
DEFAULT_CLI_SUMMARY_FILENAME = "options_portfolio_control_report_pipeline_operation_cli_summary.json"
...
def main(
    ...
    *,
    writer: WriterFunction = write_options_portfolio_control_report_pipeline_operation_files,
) -> int:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run the audited options portfolio control report pipeline operation. "
            "This command runs the file-driven pipeline and writes a top-level "
            "operation record, audit, health report, event list, and JSONL event "
            "log. It does not call brokers, route orders, submit orders, model "
    ...
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Directory where audited pipeline operation artifacts should be written.",
    )
    parser.add_argument(
        "--base-dir",
        default=None,
...
def _build_cli_summary(
    *,
    ...
    return {
        ...
        "operation_type": "options_portfolio_control_report_pipeline_operation_cli",
        ...
        "operation_summary": writer_result.get("operation_summary", {}),
        "pipeline_summary": writer_result.get("pipeline_summary", {}),
