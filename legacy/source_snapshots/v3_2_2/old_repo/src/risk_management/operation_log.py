...
class RiskManagementOperationLogError(ValueError):
    """Raised when risk management operation log persistence fails."""


def append_risk_management_operation_record(
...
def read_risk_management_operation_log(
    log_path: str | PathLike[str],
) -> list[dict[str, Any]]:
    path = Path(log_path)
    ...
    with path.open("r", encoding="utf-8") as file:
        for line_number, line in enumerate(file, start=1):
            ...
            try:
                ...
                raise RiskManagementOperationLogError(
...
def _record_to_dict(
    ...
    raise RiskManagementOperationLogError(
        "record must be a RiskManagementOperationRecord or mapping"
    )
...
def _validate_log_payload(payload: Mapping[str, Any]) -> None:
    ...
    if operation_type != RISK_MANAGEMENT_OPERATION_TYPE:
        raise RiskManagementOperationLogError(
            f"invalid operation_type for risk-management log: {operation_type!r}"
        )

    if schema_version != RISK_MANAGEMENT_OPERATION_RECORD_SCHEMA_VERSION:
        raise RiskManagementOperationLogError(
            f"invalid schema_version for risk-management log: {schema_version!r}"
        )

    if not payload.get("operation_id"):
        raise RiskManagementOperationLogError(
            "risk-management operation log payload missing operation_id"
        )
