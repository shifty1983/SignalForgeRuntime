"""QC manifest for portfolio exposure constraint sensitivity."""
...
ADAPTER_TYPE = "portfolio_exposure_constraint_sensitivity_qc_manifest_builder"
ARTIFACT_TYPE = "signalforge_portfolio_exposure_constraint_sensitivity_qc_manifest"
CONTRACT = "portfolio_exposure_constraint_sensitivity_qc_manifest"
...
def _best_by_capital(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for capital, items in grouped.items():
        passing = [row for row in items if row.get("passes_exposure_constraint_gate")]
        ...
        output[capital] = {
            ...
            "max_trades_per_date": best.get("max_trades_per_date"),
            "max_symbol_trades_per_month": best.get("max_symbol_trades_per_month"),
            "max_strategy_trades_per_month": best.get("max_strategy_trades_per_month"),
            "passes_exposure_constraint_gate": best.get("passes_exposure_constraint_gate"),
            ...
            "exposure_constraint_skip_rate": best.get("exposure_constraint_skip_rate"),
            "top_symbol_positive_contribution_pct": best.get("top_symbol_positive_contribution_pct"),
            "top_strategy_positive_contribution_pct": best.get("top_strategy_positive_contribution_pct"),
        }
    return output
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if summary.get("contract") != "portfolio_exposure_constraint_sensitivity":
    ...
    failed_missing_reasons = [
        ...
        if not row.get("passes_exposure_constraint_gate") and not row.get("failure_reasons")
    ]
    ...
    if missing_passing_capitals:
        warnings.append("some_starting_capitals_have_no_passing_exposure_variant")

    gate_failure_counts = Counter(reason for row in scenarios for reason in row.get("failure_reasons", []))
    exposure_skip_counts = {
        "date": sum(int(row.get("skipped_exposure_date_count") or 0) for row in scenarios),
        "symbol_month": sum(int(row.get("skipped_exposure_symbol_month_count") or 0) for row in scenarios),
        "strategy_month": sum(int(row.get("skipped_exposure_strategy_month_count") or 0) for row in scenarios),
    }
    ...
    manifest_path = output_dir / "signalforge_portfolio_exposure_constraint_sensitivity_qc_manifest.json"
    manifest: dict[str, Any] = {
        ...
        "diagnostics": {
            ...
            "aggregate_exposure_skip_counts": exposure_skip_counts,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build QC manifest for portfolio exposure constraint sensitivity.")
