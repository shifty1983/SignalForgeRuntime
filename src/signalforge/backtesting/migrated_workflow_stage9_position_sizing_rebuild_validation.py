...
GENERATED_DIR = OUTPUT_ROOT / "stage9_position_sizing_rebuild"
GENERATED_ROWS = GENERATED_DIR / "signalforge_portfolio_position_sizing_replay.jsonl"
GENERATED_SUMMARY = GENERATED_DIR / "signalforge_portfolio_position_sizing_replay_summary.json"
...
def build_stage9_position_sizing_rebuild_validation() -> dict[str, Any]:
    blockers: list[str] = []
    ...
    if blockers:
        return {
            "adapter_type": "migrated_workflow_stage9_position_sizing_rebuild_validation_builder",
            "artifact_type": "signalforge_migrated_workflow_stage9_position_sizing_rebuild_validation",
            "is_ready": False,
    ...
    if source_count != generated_count:
        blockers.append("position_sizing_row_count_mismatch")

    if source_keys != generated_keys:
        blockers.append("position_sizing_sample_schema_mismatch")
    ...
    return {
        ...
        "generated_summary_core": {
            ...
            "input_trade_count": summary.get("input_trade_count"),
            "output_row_count": summary.get("output_row_count"),
            "starting_equity": summary.get("starting_equity"),
            "risk_per_trade_pct": summary.get("risk_per_trade_pct"),
            "max_trade_risk_dollars": summary.get("max_trade_risk_dollars"),
        },
        "blocker_count": len(blockers),
        "blockers": blockers,
        "is_ready": len(blockers) == 0,
        "readiness_scope": "migrated_position_sizing_rebuild_matches_source_row_count_schema_and_hash",
    }


def main() -> int:
    result = build_stage9_position_sizing_rebuild_validation()

    output_path = OUTPUT_ROOT / "signalforge_migrated_workflow_stage9_position_sizing_rebuild_validation.json"
