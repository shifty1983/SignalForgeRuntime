...
GENERATED_DIR = OUTPUT_ROOT / "stage7_strategy_selection_rebuild"
GENERATED_ROWS = GENERATED_DIR / "signalforge_historical_strategy_selection_rows.jsonl"
GENERATED_SUMMARY = GENERATED_DIR / "signalforge_historical_strategy_selection_rows_summary.json"
...
def build_stage7_strategy_selection_rebuild_validation() -> dict[str, Any]:
    blockers: list[str] = []
    ...
    if blockers:
        return {
            "adapter_type": "migrated_workflow_stage7_strategy_selection_rebuild_validation_builder",
            "artifact_type": "signalforge_migrated_workflow_stage7_strategy_selection_rebuild_validation",
            "is_ready": False,
    ...
    if source_count != generated_count:
        blockers.append("selection_row_count_mismatch")

    if source_keys != generated_keys:
        blockers.append("selection_sample_schema_mismatch")
    ...
    return {
        ...
        "readiness_scope": "migrated_strategy_selection_rebuild_matches_source_row_count_schema_and_hash",
    }


def main() -> int:
    result = build_stage7_strategy_selection_rebuild_validation()

    output_path = OUTPUT_ROOT / "signalforge_migrated_workflow_stage7_strategy_selection_rebuild_validation.json"
