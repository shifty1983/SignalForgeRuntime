...
from signalforge.engines.regime.fred_pipeline import build_signalforge_fred_regime_pipeline
...
def _read_macro_parquet_rows(root: Path) -> list[dict[str, Any]]:
    """Read local FRED parquet rows and drop FRED missing-value observations.

    FRED uses "." for missing observations such as holidays. In parquet these can
    land as null values. The regime builder requires numeric value rows only.
    """
    rows: list[dict[str, Any]] = []

    if not root.exists():
        raise ValueError(f"Macro parquet root does not exist: {root}")
    ...
    if not rows:
        raise ValueError(f"No numeric macro parquet rows found under: {root}")
