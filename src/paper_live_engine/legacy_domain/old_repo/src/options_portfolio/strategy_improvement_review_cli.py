...
from src.options_portfolio.strategy_improvement_review_file_writer import (
    write_options_strategy_improvement_review_operation_files,
)


CLI_SUMMARY_SCHEMA_VERSION = "options_strategy_improvement_review_cli_summary.v1"
DEFAULT_CLI_SUMMARY_FILENAME = "options_strategy_improvement_review_cli_summary.json"
...
def main(
    ...
    *,
    writer: WriterFunction = write_options_strategy_improvement_review_operation_files,
) -> int:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Build local options strategy improvement review artifacts from options "
            "strategy improvement queue outputs. This command writes local files only "
    ...
    parser.add_argument(
        "--source",
        required=True,
        help="Path to options strategy improvement review source JSON.",
    )
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Directory where options strategy improvement review artifacts should be written.",
    )
    parser.add_argument(
        "--review-date",
        default=None,
...
def _build_cli_summary(
    *,
    ...
    return {
        ...
        "operation_type": "options_strategy_improvement_review_cli",
