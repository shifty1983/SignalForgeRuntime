...
from src.options_portfolio.action_execution_record_file_writer import (
    write_options_manual_execution_record_operation_files,
)


CLI_SUMMARY_SCHEMA_VERSION = "options_manual_execution_record_cli_summary.v1"
DEFAULT_CLI_SUMMARY_FILENAME = "options_manual_execution_record_cli_summary.json"
...
def main(
    ...
    *,
    writer: WriterFunction = write_options_manual_execution_record_operation_files,
) -> int:
    ...
    writer_result = writer(
        ...
        execution_recorded_at=args.execution_recorded_at,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Build local options manual execution-record operation artifacts from "
            "a manual action review and manual execution records. This command "
            ...
            "or create automatic close/roll/defense orders."
        )
    )
    parser.add_argument(
        "--source",
        required=True,
        help="Path to options manual execution-record source JSON.",
    )
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Directory where manual execution-record artifacts should be written.",
    )
    parser.add_argument(
        "--execution-recorded-at",
        default=None,
        help=(
            "Optional execution-record timestamp override. If omitted, "
            "source.execution_recorded_at is used."
        ),
    )
    parser.add_argument(
        "--recorder",
        default=None,
        help="Optional recorder label override. If omitted, source.recorder is used.",
    )
    return parser
...
def _build_cli_summary(
    *,
    ...
    return {
        ...
        "operation_type": "options_manual_execution_record_cli",
