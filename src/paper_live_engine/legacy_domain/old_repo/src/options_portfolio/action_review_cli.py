...
from src.options_portfolio.action_review_file_writer import (
    write_options_manual_action_review_operation_files,
)


CLI_SUMMARY_SCHEMA_VERSION = "options_manual_action_review_cli_summary.v1"
DEFAULT_CLI_SUMMARY_FILENAME = "options_manual_action_review_cli_summary.json"
...
def main(
    ...
    *,
    writer: WriterFunction = write_options_manual_action_review_operation_files,
) -> int:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Build local options manual action-review operation artifacts from "
            "a manual action queue and human review decisions. This command "
    ...
    parser.add_argument(
        "--source",
        required=True,
        help="Path to options manual action-review source JSON.",
    )
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Directory where manual action-review artifacts should be written.",
    )
    parser.add_argument(
        "--reviewed-at",
        default=None,
        help="Optional review timestamp override. If omitted, source.reviewed_at is used.",
    )
    parser.add_argument(
        "--reviewer",
        default=None,
        help="Optional reviewer label override. If omitted, source.reviewer is used.",
    )
    return parser
...
def _build_cli_summary(
    *,
    ...
    return {
        ...
        "operation_type": "options_manual_action_review_cli",
