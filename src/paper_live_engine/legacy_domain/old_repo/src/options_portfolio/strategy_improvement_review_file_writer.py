...
from src.options_portfolio.strategy_improvement_review_operation import (
    run_options_strategy_improvement_review_operation,
)


FILE_WRITER_SCHEMA_VERSION = "options_strategy_improvement_review_files.v1"
OPERATION_TYPE = "options_strategy_improvement_review_file_writer"

DEFAULT_FILENAMES = {
    "options_strategy_improvement_review": "options_strategy_improvement_review.json",
    "operation_result": "options_strategy_improvement_review_operation.json",
    "audit_report": "options_strategy_improvement_review_audit.json",
    "health_report": "options_strategy_improvement_review_health.json",
    "events": "options_strategy_improvement_review_events.json",
    "event_log": "options_strategy_improvement_review_operation.jsonl",
}


def write_options_strategy_improvement_review_operation_files(
    source: Any,
    *,
    ...
    operation_result = run_options_strategy_improvement_review_operation(
        source_args["operation_source"],
        metadata=source_args["metadata"],
        event_log_path=event_log_path,
    )

    review = _as_mapping(operation_result.get("options_strategy_improvement_review"))
    ...
    files = {
        "options_strategy_improvement_review": output_path / DEFAULT_FILENAMES["options_strategy_improvement_review"],
    ...
    _write_json(files["options_strategy_improvement_review"], review)
...
def _extract_source_args(source: Any, *, review_date: str | None) -> dict[str, Any]:
    ...
    if queues is not None:
        operation_source["options_strategy_improvement_queues"] = queues

    single_queue = _extract_first_mapping(
        source,
        "options_strategy_improvement_queue",
        "strategy_improvement_queue",
        "queue",
    )
    if single_queue is not None:
        operation_source["options_strategy_improvement_queue"] = single_queue
...
def _build_source_summary(source: Any) -> dict[str, Any]:
    if not isinstance(source, Mapping):
        return {
            ...
            "review_date": None,
            "has_strategy_improvement_queue": False,
            "queue_count": 0,
            "single_queue_present": False,
        }

    source_is_queue = source.get("artifact_type") == "options_strategy_improvement_queue"
    queues = _as_list(source.get("options_strategy_improvement_queues"))
    single_queue = _as_mapping(source.get("options_strategy_improvement_queue"))
    queue_count = len(queues) + (1 if single_queue or source_is_queue else 0)

    return {
        "source_shape": "mapping",
        "review_date": _string_or_none(source.get("review_date")),
        "has_strategy_improvement_queue": queue_count > 0,
        "queue_count": queue_count,
        "single_queue_present": bool(single_queue or source_is_queue),
    }
