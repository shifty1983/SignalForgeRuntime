...
from src.options_portfolio.action_outcome_record_file_writer import (
    write_options_manual_action_outcome_record_operation_files,
)


CLI_SUMMARY_SCHEMA_VERSION = "options_manual_action_outcome_record_cli_summary.v1"
DEFAULT_CLI_SUMMARY_FILENAME = "options_manual_action_outcome_record_cli_summary.json"
...
def main(
    ...
    *,
    writer: WriterFunction = write_options_manual_action_outcome_record_operation_files,
) -> int:
    ...
    writer_result = writer(
        ...
        outcome_recorded_at=args.outcome_recorded_at,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Build local options manual action-outcome operation artifacts from "
            "a manual execution record and manual outcome records. This command "
    ...
    parser.add_argument(
        "--source",
        required=True,
        help="Path to options manual action-outcome source JSON.",
    )
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Directory where manual action-outcome artifacts should be written.",
    )
    parser.add_argument(
        "--outcome-recorded-at",
        default=None,
        help=(
            "Optional outcome-record timestamp override. If omitted, "
            "source.outcome_recorded_at is used."
        ),
    )
    parser.add_argument(
        "--recorder",
...
def _build_cli_summary(
    *,
    ...
    return {
        ...
        "operation_type": "options_manual_action_outcome_record_cli",
